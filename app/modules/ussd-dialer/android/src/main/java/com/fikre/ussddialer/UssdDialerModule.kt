...
class UssdDialerModule : Module() {
  override fun definition() = ModuleDefinition {
    ...
    AsyncFunction("configureBackend") { baseUrl: String, enrollmentKey: String, deviceName: String ->
      val context = appContext.reactContext ?: throw IllegalStateException("Android context is unavailable")
      if (!BackendListenerService.isFullyStopped()) throw IllegalStateException("Stop the backend listener before changing its configuration")
      val cleanUrl = BackendController.validateBaseUrl(baseUrl)
      val currentBackend = UssdAutomationStore.backendConfiguration(context)
      val hasPendingJob = UssdAutomationStore.isBackendPendingJobPresent(context)
      val refreshingExpiredCredentials = hasPendingJob && currentBackend["state"] == "auth_expired"
      if (hasPendingJob && !refreshingExpiredCredentials) {
        throw IllegalStateException("A backend request is still pending and must finish before configuration can change")
      }
      if (refreshingExpiredCredentials && currentBackend["baseUrl"]?.toString() != cleanUrl) {
        throw IllegalStateException("Reconnect to the same backend URL until its pending request is reconciled")
      }
      val cleanKey = enrollmentKey.trim()
      val cleanName = deviceName.trim()
      if (cleanKey.isEmpty() || cleanKey.length > 512 || cleanKey.any(Char::isISOControl)) {
        throw IllegalArgumentException("Enter a valid enrollment key")
      }
      if (cleanName.isEmpty() || cleanName.length > 80 || cleanName.any(Char::isISOControl)) {
        throw IllegalArgumentException("Device name must contain 1 to 80 printable characters")
      }
      val proposedId = BackendController.deviceId(context)
      val (registeredId, token) = BackendHttp.register(cleanUrl, cleanKey, proposedId, cleanName)
      val saved = if (refreshingExpiredCredentials) {
        UssdAutomationStore.refreshBackendCredentials(context, cleanUrl, registeredId, cleanName, token)
      } else {
        UssdAutomationStore.saveBackendConfiguration(context, cleanUrl, registeredId, cleanName, token)
      }
      if (!saved) {
        throw IllegalStateException("Backend credentials could not be saved securely")
      }
      BackendController.status(context)
    }

    AsyncFunction("getBackendStatus") {
      val context = appContext.reactContext ?: throw IllegalStateException("Android context is unavailable")
      BackendController.status(context)
    }

    AsyncFunction("startBackendListener") {
      val context = appContext.reactContext ?: throw IllegalStateException("Android context is unavailable")
      val activity = appContext.currentActivity
      if (activity == null || activity.isFinishing) {
        throw IllegalStateException("Open USSD Flow before starting its background listener")
      }
      BackendController.start(context)
      BackendController.status(context)
    }

    AsyncFunction("stopBackendListener") {
      val context = appContext.reactContext ?: throw IllegalStateException("Android context is unavailable")
      BackendController.stop(context)
      BackendController.status(context)
    }

    AsyncFunction("clearBackendConfiguration") {
      val context = appContext.reactContext ?: throw IllegalStateException("Android context is unavailable")
      if (UssdAutomationStore.isBackendPendingJobPresent(context)) {
        throw IllegalStateException("A backend request is still pending and cannot be discarded safely")
      }
      BackendController.stop(context)
      if (!UssdAutomationStore.clearBackendConfiguration(context)) {
        throw IllegalStateException("Backend configuration could not be removed securely")
      }
      BackendController.status(context)
    }

    AsyncFunction("syncBackendCatalog") {
      val context = appContext.reactContext ?: throw IllegalStateException("Android context is unavailable")
      BackendController.requestCatalogSync(context)
      BackendController.status(context)
    }

    AsyncFunction("getBackendJobs") {
      val context = appContext.reactContext ?: throw IllegalStateException("Android context is unavailable")
      BackendController.listQueuedJobs(context)
    }

    AsyncFunction("deleteBackendJob") { jobId: String ->
      val context = appContext.reactContext ?: throw IllegalStateException("Android context is unavailable")
      BackendController.cancelQueuedJob(context, jobId)
    }
    ...
    AsyncFunction("startAutomation") { code: String, replies: List<String>, subscriptionId: Int, flowName: String, promise: Promise ->
      ...
      try {
        val result = UssdAutomationLauncher.start(context, code, replies, subscriptionId, flowName)
        if (result.started) {
          promise.resolve(null)
        } else {
          promise.reject(result.code, result.message, null)
        }
      } catch (error: Exception) {
    ...
    AsyncFunction("saveFlow") { name: String, code: String, replies: List<String>, subscriptionId: Int ->
      ...
      FlowTemplates.validateSavedReplies(normalizedReplies).error?.let { throw IllegalArgumentException(it) }
